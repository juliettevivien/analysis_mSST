...
def load_behav_data(
    ...
    for subject in included_subjects:
        ...
        if subject.startswith('sub'):
            subject_ID, na, condition, task = subject.split(' ')
            sub_path = join(data_path, subject_ID)
            sub_path_condition = join(sub_path, (na + ' ' + condition))
            behav_filename = [f for f in os.listdir(sub_path_condition) if f.endswith('.csv')]
            filepath = join(sub_path_condition, behav_filename[0])
            df = pd.read_csv(filepath)
            
        elif subject.startswith('C'):
            subject_ID, task = subject.split(' ')
            sub_path = join(data_path, subject_ID)
            behav_filename = [f for f in os.listdir(sub_path) if f.endswith('.csv')]
            filepath = join(sub_path, behav_filename[0])
            df = pd.read_csv(filepath)
