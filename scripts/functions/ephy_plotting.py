...
from functions.analysis import compute_percent_change, identify_significant_clusters
...
def perc_pow_diff_on_off(
    ...
    for sub in sub_dict_ON_OFF.keys(): # sub006, sub009, sub015, sub019
        ...
        for subject, epochs in sub_dict_ON_OFF[sub].items():
            ...
            if 'DBS ON' in subject:
                ...
                if not (cond == 'GS_successful' or cond == 'STOP_successful'):
                    RT_ON = RT_dict_ON_OFF[sub][subject][cond]
            ...
            if 'DBS OFF' in subject:
                data_OFF = epochs[cond]
                if not (cond == 'GS_successful' or cond == 'STOP_successful'):
                    RT_OFF = RT_dict_ON_OFF[sub][subject][cond]
        ...
        #diff_left = percentage_change_left_OFF - percentage_change_left_ON
        diff_right = percentage_change_right_ON - percentage_change_right_OFF
        #diff_right = percentage_change_right_OFF - percentage_change_right_ON
...
def plot_raw_stim(
    ...
    ax[0,0].plot(timescale, L_chan, label=raw.ch_names[0])
    ax[0,0].set_title(raw.ch_names[0])
    ax[0,0].set_ylabel('LFP amplitude')
    ax[1,0].plot(timescale, stim_L_chan, color = 'orange', label='Stim left')
    ax[1,0].set_ylabel('Stimulation amplitude')
    ax[1,0].set_xlabel('Time [s]')

    #ax[0,1].legend()
    ax[0,1].plot(timescale, R_chan, label=raw.ch_names[1])
